...
import { testPlacemarks, testMarkers, dublin, cork, concert, testUsers } from "../fixtures.js";
...
suite("Marker Model tests", () => {
  ...
  setup(async () => {
    ...
    for (let i = 0; i < testMarkers.length; i += 1) {
      ...
      testMarkers[i] = await db.markerStore.addTrack(dublinList._id, testMarkers[i]);
    }
  });

  test("create single Marker", async () => {
    const concertList = await db.placemarkStore.addPlacemark(concert);
    const marker = await db.markerStore.addMarker(concertList._id, cork)
    assert.isNotNull(marker._id);
    assertSubset (cork, marker);
  });
